...
async def main():
    ...
    print(f"Prijs in USDC per HBAR: {prijs_usdc_per_hbar:.6f}")
    ...
    if hbar_raw <= 0:
        print("\nWAARSCHUWING: berekende HBAR-inzet is 0 of negatief. GESTOPT.")
        await db.close()
        return

    print("\n=== Stap 4: positie openen (marge verruimd naar 20%) ===")
    lp.open_position(
        hbar_raw, usdc_raw, prijs_usdc_per_hbar, slippage_tolerance=0.20,
